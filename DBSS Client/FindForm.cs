...
namespace ServiceStation
{
	...
	public partial class FinderForm : Form
	{
		...
		private void FinderFormLoader(object sender, EventArgs e)
		{
			...
			toolFindBox.Text = $"Search in the \"{ TableName }\" of:";
			...
			{
				case "Cars":
					foreach (object col in dbssDS.Cars.Columns)
					{
						columnBox.Items.Add(col.ToString());
					}
					break;
				case "CarOwners":
					foreach (object col in dbssDS.CarOwners.Columns)
					{
						columnBox.Items.Add(col.ToString());
					}
					break;
				case "Manufacturers":
					foreach (object col in dbssDS.Manufacturers.Columns)
					{
						columnBox.Items.Add(col.ToString());
					}
					break;
				case "Applications":
					foreach (object col in dbssDS.Applications.Columns)
					{
						columnBox.Items.Add(col.ToString());
					}
					break;
				case "Masters":
					foreach (object col in dbssDS.Masters.Columns)
					{
						columnBox.Items.Add(col.ToString());
					}
					break;
				case "Services":
					foreach (object col in dbssDS.Services.Columns)
					{
						columnBox.Items.Add(col.ToString());
					}
					break;
				case "Data":
					foreach (object col in dbssDS.DATA.Columns)
					{
						columnBox.Items.Add(col.ToString());
					}
					break;
